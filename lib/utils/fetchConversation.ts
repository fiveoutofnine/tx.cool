...
const fetchConversation = async (
  ...
  if (
    ...
    const response = await fetch('https://api.transpose.io/sql', {
      ...
      sql: `SELECT t.block_number, t.from_address, t.to_address, t.value, raw.message, t.timestamp, t.transaction_hash FROM ( SELECT block_number, from_address, to_address, value, input, timestamp, transaction_hash FROM ethereum.transactions WHERE input IS NOT NULL AND ((from_address = \'{{address1}}\' AND to_address = \'{{address2}}\') OR (to_address = \'{{address1}}\' AND from_address = \'{{address2}}\')) ORDER BY block_number DESC ) AS t, LATERAL ( SELECT CONVERT_FROM( DECODE( SUBSTRING( REGEXP_REPLACE(t.input, \'00\', \'\', \'g\') FROM 3 ), \'hex\' ), \'UTF-8\' ) AS message ) AS raw WHERE LENGTH(raw.message) > 0 AND ( LENGTH(raw.message) - LENGTH(REGEXP_REPLACE(raw.message, '[\\u002D\\u0030-\\u0039\\u0041-\\u005A\\u005F\\u0061-\\u007A!@#$%^&*()_+<>]', '', 'g')) )::float / LENGTH(raw.message) >= 0.5 LIMIT 100 OFFSET {{offset}}`,
