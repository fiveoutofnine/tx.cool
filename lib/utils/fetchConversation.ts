...
const fetchConversation = cache(
  async (address1: Address, address2: Address, page: number): Promise<ChatMessageTx[]> => {
    const minAddress =
    ...
    if (
      ...
      sql: `SELECT t.block_number, t.from_address, t.to_address, t.value, raw.message, t.timestamp, t.transaction_hash FROM ( SELECT block_number, from_address, to_address, value, input, timestamp, transaction_hash FROM ethereum.transactions WHERE input IS NOT NULL AND ((from_address = \'{{address1}}\' AND to_address = \'{{address2}}\') OR (to_address = \'{{address1}}\' AND from_address = \'{{address2}}\')) ORDER BY block_number DESC ) AS t, LATERAL ( SELECT CONVERT_FROM( DECODE( SUBSTRING( REGEXP_REPLACE(t.input, \'00\', \'\', \'g\') FROM 3 ), \'hex\' ), \'LATIN1\' ) AS message ) AS raw WHERE raw.message ~ \'\\A[\\sa-zA-Z0-9%&()\\[\\]#@.,;:?!_-]*\\Z\' LIMIT 100 OFFSET {{offset}}`,
